...
w3 = Web3(scroll_alpha)


def deploy_contract(wallet, contract):
    transaction = contract.constructor(wallet.address).buildTransaction(
        {
...
def load_wallets() -> list[LocalAccount]:
    ...
    file = Path("./wallets/Private_key.txt").open()
...
def load_bytecode() -> str:
    ...
    return Path("./contractGames/contract_byte_code.txt").open().readline()
...
def main() -> None:
    ...
    contract = w3.eth.contract(
        bytecode=bytecode,
        abi=json.load(Path("./contractGames/contract_abi.json").open())
    )
